...
# ====================== CẤU HÌNH ======================
GROUP_URL = "https://www.facebook.com/hoinguoixauvietnam"
TARGET = 50
...
# Dedup bằng image ID thay vì full URL (tránh lỗi CDN token đổi)
seen_urls = set(
...
def get_text_from_post(post_el):
    try:
        ...
        print(f"⚠️ Lỗi lấy text chi tiết: {e}")
        return ""
...
with sync_playwright() as p:
    ...
    # ✅ Chờ feed render đầy đủ trước khi bắt đầu quét
    ...
    while collected < TARGET:
        # ✅ Quét TRƯỚC, scroll SAU — không bỏ sót post đầu trang
        ...
        if new_this_round == 0:
            no_new_rounds += 1
            print(f"⚠️ Không có post mới ({no_new_rounds}/15)")
            if no_new_rounds >= 15:
                print("🏁 Dừng vì không còn post mới.")
                break
            # Scroll nhiều hơn khi bị stuck
            human_scroll(page, times=6)
        else:
            no_new_rounds = 0
            # Scroll ít thôi để không bỏ sót post giữa 2 lần quét
